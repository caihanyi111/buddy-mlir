//===- launch.c - Board/host launcher for embedding_w8a8_1x1024 -----------===//
//
...
extern void _mlir_ciface_kernel_embedding_w8a8_1x1024(MemRef1 *, MemRef2 *,
                                                      MemRef2 *, MemRef1 *);
int launch(void) {
  int errors = 0;
  float maximum = 0;
  ...
  _mlir_ciface_kernel_embedding_w8a8_1x1024(&m_ids, &m_out, &m_weight,
                                            &m_scale);
  for (int s = 0; s < 1; ++s)
    for (int k = 0; k < 1024; ++k) {
      float expected =
