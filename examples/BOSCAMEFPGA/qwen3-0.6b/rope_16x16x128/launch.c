//===- launch.c - Board/host launcher for rope_16x16x128 ------------------===//
//
...
extern void _mlir_ciface_kernel_rope_16x16x128(MemRef4 *, MemRef2 *, MemRef2 *,
                                               MemRef4 *);
int launch(void) {
  int errors = 0;
  float maximum = 0;
  float *x = (float *)workspace(0u);
  MemRef4 m_x = make_4(x, 16, 16, 2, 64);
  float *cosine = (float *)workspace(131072u);
  MemRef2 m_cosine = make_2(cosine, 16, 64);
  float *sine = (float *)workspace(135168u);
  MemRef2 m_sine = make_2(sine, 16, 64);
  float *out = (float *)workspace(139264u);
  MemRef4 m_out = make_4(out, 16, 16, 2, 64);
  for (int i = 0; i < 32768; ++i) {
    x[i] = (float)((i * 5) % 53 - 26) * 0.03125f;
    out[i] = 9999;
  }
  for (int s = 0; s < 16; ++s)
    for (int k = 0; k < 64; ++k) {
      float frequency = oracle_exp(-13.815510557964274f * (float)k / 64.0f);
      float angle = (float)(s + 7) * frequency;
      cosine[s * 64 + k] = oracle_cos(angle);
      sine[s * 64 + k] = oracle_sin(angle);
    }
  _mlir_ciface_kernel_rope_16x16x128(&m_x, &m_cosine, &m_sine, &m_out);
  for (int s = 0; s < 16; ++s)
    for (int h = 0; h < 16; ++h)
      for (int k = 0; k < 64; ++k) {
        int first = (s * 16 + h) * 128 + k, second = first + 64;
        float a = x[first] * cosine[s * 64 + k] - x[second] * sine[s * 64 + k];
        float b = x[second] * cosine[s * 64 + k] + x[first] * sine[s * 64 + k];
        compare(out[first], a, 2e-6f, 3e-5f, &errors, &maximum);
        compare(out[second], b, 2e-6f, 3e-5f, &errors, &maximum);
      }
  return print_check("rope_16x16x128", errors, maximum);
}
