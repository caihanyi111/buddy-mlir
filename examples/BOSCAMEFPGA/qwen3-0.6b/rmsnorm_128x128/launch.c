//===- launch.c - Board/host launcher for rmsnorm_128x128 -----------------===//
//
...
extern void _mlir_ciface_kernel_rmsnorm_128x128(MemRef2 *, MemRef1 *, MemRef1 *,
                                                MemRef2 *);
int launch(void) {
  int errors = 0;
  float maximum = 0;
  float *x = (float *)workspace(0u);
  MemRef2 m_x = make_2(x, 128, 128);
  float *weight = (float *)workspace(65536u);
  MemRef1 m_weight = make_1(weight, 128);
  float *sums = (float *)workspace(66048u);
  MemRef1 m_sums = make_1(sums, 128);
  float *out = (float *)workspace(66560u);
  MemRef2 m_out = make_2(out, 128, 128);
  ...
  _mlir_ciface_kernel_rmsnorm_128x128(&m_x, &m_weight, &m_sums, &m_out);
  for (int r = 0; r < 128; ++r) {
    oracle_float sum = 0;
    for (int k = 0; k < 128; ++k) {
  ...
  return print_check("rmsnorm_128x128", errors, maximum);
}
