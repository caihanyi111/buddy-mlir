//===- launch.c - Board/host launcher for rmsnorm_16x1024 -----------------===//
//
...
extern void _mlir_ciface_kernel_rmsnorm_16x1024(MemRef2 *, MemRef1 *, MemRef1 *,
                                                MemRef2 *);
int launch(void) {
  int errors = 0;
  float maximum = 0;
  float *x = (float *)workspace(0u);
  MemRef2 m_x = make_2(x, 16, 1024);
  float *weight = (float *)workspace(65536u);
  MemRef1 m_weight = make_1(weight, 1024);
  float *sums = (float *)workspace(69632u);
  MemRef1 m_sums = make_1(sums, 16);
  float *out = (float *)workspace(69696u);
  MemRef2 m_out = make_2(out, 16, 1024);
  ...
  _mlir_ciface_kernel_rmsnorm_16x1024(&m_x, &m_weight, &m_sums, &m_out);
  for (int r = 0; r < 16; ++r) {
    oracle_float sum = 0;
    for (int k = 0; k < 1024; ++k) {
      oracle_float value = x[r * 1024 + k];
      sum += value * value;
    }
    float denominator = oracle_sqrt((float)(sum / 1024) + 1e-6f);
    for (int k = 0; k < 1024; ++k) {
      int i = r * 1024 + k;
  ...
  return print_check("rmsnorm_16x1024", errors, maximum);
}
