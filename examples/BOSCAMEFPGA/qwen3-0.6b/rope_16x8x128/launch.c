//===- launch.c - Board/host launcher for rope_16x8x128 -------------------===//
//
...
extern void _mlir_ciface_kernel_rope_16x8x128(MemRef4 *, MemRef2 *, MemRef2 *,
                                              MemRef4 *);
int launch(void) {
  int errors = 0;
  float maximum = 0;
  float *x = (float *)workspace(0u);
  MemRef4 m_x = make_4(x, 16, 8, 2, 64);
  float *cosine = (float *)workspace(65536u);
  MemRef2 m_cosine = make_2(cosine, 16, 64);
  float *sine = (float *)workspace(69632u);
  MemRef2 m_sine = make_2(sine, 16, 64);
  float *out = (float *)workspace(73728u);
  MemRef4 m_out = make_4(out, 16, 8, 2, 64);
  ...
  _mlir_ciface_kernel_rope_16x8x128(&m_x, &m_cosine, &m_sine, &m_out);
  for (int s = 0; s < 16; ++s)
    for (int h = 0; h < 8; ++h)
      for (int k = 0; k < 64; ++k) {
  ...
  return print_check("rope_16x8x128", errors, maximum);
}
