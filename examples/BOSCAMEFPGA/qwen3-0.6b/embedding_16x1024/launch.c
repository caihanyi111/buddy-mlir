//===- launch.c - Board/host launcher for embedding_16x1024 ---------------===//
//
...
extern void _mlir_ciface_kernel_embedding_16x1024(MemRef1 *, MemRef2 *,
                                                  MemRef2 *);
int launch(void) {
  int errors = 0;
  float maximum = 0;
  ...
  float *weight = (float *)workspace(65664u);
  ...
  _mlir_ciface_kernel_embedding_16x1024(&m_ids, &m_out, &m_weight);
  for (int s = 0; s < 16; ++s)
    for (int k = 0; k < 1024; ++k) {
      float expected = (float)((ids[s] + k * 3) % 97 - 48) * 0.03125f;
      compare(out[s * 1024 + k], expected, 0, 0, &errors, &maximum);
    }
  return print_check("embedding_16x1024", errors, maximum);
}
