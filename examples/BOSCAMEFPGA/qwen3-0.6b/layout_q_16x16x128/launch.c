//===- launch.c - Board/host launcher for layout_q_16x16x128 --------------===//
//
...
extern void _mlir_ciface_kernel_layout_q_16x16x128(MemRef3 *, MemRef3 *);
int launch(void) {
  int errors = 0;
  float maximum = 0;
  ...
  _mlir_ciface_kernel_layout_q_16x16x128(&m_x, &m_out);
  for (int a = 0; a < 16; ++a)
    for (int b = 0; b < 16; ++b)
      for (int d = 0; d < 128; ++d) {
        int index[3] = {a, b, d};
  ...
  return print_check("layout_q_16x16x128", errors, maximum);
}
