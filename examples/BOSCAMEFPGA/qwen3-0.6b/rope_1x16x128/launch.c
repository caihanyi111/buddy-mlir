//===- launch.c - Board/host launcher for rope_1x16x128 -------------------===//
//
...
extern void _mlir_ciface_kernel_rope_1x16x128(MemRef4 *, MemRef2 *, MemRef2 *,
                                              MemRef4 *);
int launch(void) {
  int errors = 0;
  float maximum = 0;
  float *x = (float *)workspace(0u);
  MemRef4 m_x = make_4(x, 1, 16, 2, 64);
  float *cosine = (float *)workspace(8192u);
  MemRef2 m_cosine = make_2(cosine, 1, 64);
  float *sine = (float *)workspace(8448u);
  MemRef2 m_sine = make_2(sine, 1, 64);
  float *out = (float *)workspace(8704u);
  MemRef4 m_out = make_4(out, 1, 16, 2, 64);
  ...
  _mlir_ciface_kernel_rope_1x16x128(&m_x, &m_cosine, &m_sine, &m_out);
  for (int s = 0; s < 1; ++s)
    for (int h = 0; h < 16; ++h)
      for (int k = 0; k < 64; ++k) {
  ...
  return print_check("rope_1x16x128", errors, maximum);
}
