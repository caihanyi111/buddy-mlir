//===- launch.c - Board/host launcher for dequantize_16x2048 --------------===//
//
...
#define M 16
#define N 2048
extern void _mlir_ciface_kernel_dequantize_16x2048(MemRef2 *, MemRef1 *,
                                                   MemRef1 *, MemRef2 *);
int launch(void) {
  ...
  _mlir_ciface_kernel_dequantize_16x2048(&X, &R, &S, &Y);
  unsigned errors = 0;
  float max_error = 0;
  for (int i = 0; i < M; i++)
    for (int j = 0; j < N; j++) {
  ...
  return print_check("dequantize_16x2048", errors, max_error);
}
