//===- launch.c - Board/host launcher for embedding_w8a8_16x1024 ----------===//
//
...
extern void _mlir_ciface_kernel_embedding_w8a8_16x1024(MemRef1 *, MemRef2 *,
                                                       MemRef2 *, MemRef1 *);
int launch(void) {
  int errors = 0;
  float maximum = 0;
  int64_t *ids = (int64_t *)workspace(0u);
  MemRef1 m_ids = make_1(ids, 16);
  float *out = (float *)workspace(128u);
  MemRef2 m_out = make_2(out, 16, 1024);
  int8_t *weight = (int8_t *)workspace(65664u);
  MemRef2 m_weight = make_2(weight, 151936, 1024);
  float *scale = (float *)workspace(155648128u);
  MemRef1 m_scale = make_1(scale, 151936);
  for (int s = 0; s < 16; ++s) {
    ids[s] = (s % 4 == 0) ? 151935 : ((s * 997 + 3) % 151936);
    scale[ids[s]] = (float)((ids[s] % 13) + 1) * 0.0078125f;
    for (int k = 0; k < 1024; ++k) {
      weight[ids[s] * 1024 + k] = (int8_t)(((ids[s] + k * 3) % 255) - 127);
      out[s * 1024 + k] = 9999;
    }
  }
  _mlir_ciface_kernel_embedding_w8a8_16x1024(&m_ids, &m_out, &m_weight,
                                             &m_scale);
  for (int s = 0; s < 16; ++s)
    for (int k = 0; k < 1024; ++k) {
      float expected =
          (float)(int8_t)(((ids[s] + k * 3) % 255) - 127) * scale[ids[s]];
      compare(out[s * 1024 + k], expected, 0, 0, &errors, &maximum);
    }
  return print_check("embedding_w8a8_16x1024", errors, maximum);
}
