//===- launch.c - Board/host launcher for layout_context_8x16x128 ---------===//
//
...
int launch(void) {
  ...
  MemRef3 m_out = make_3(out, 16, 8, 128);
  for (int i = 0; i < 16384; ++i) {
    x[i] = (float)((i * 19) % 4093 - 2046) * 0.015625f;
    out[i] = 9999;
  }
  _mlir_ciface_kernel_layout_context_8x16x128(&m_x, &m_out);
  for (int a = 0; a < 8; ++a)
    for (int b = 0; b < 16; ++b)
      for (int d = 0; d < 128; ++d) {
        int index[3] = {a, b, d};
  ...
  return print_check("layout_context_8x16x128", errors, maximum);
}
