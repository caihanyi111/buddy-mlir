...
def configuration():
    ...
    values = {
        key[len("QWEN_CFG_") :]: value
        for key, value in os.environ.items()
        if key.startswith("QWEN_CFG_")
    }
    if not values:
        raise SystemExit(
            "build configuration must be requested through make print-config"
        )
    tool_names = (
        "BUDDY_OPT",
        "BUDDY_TRANSLATE",
    ...
    return {
        ...
        "variables": values,
        "tools": identities,
        "configuration_helper_sha256": hashlib.sha256(
...
def configuration_helper():
    ...
    if sys.argv[1] == "--audit-lowering":
        if len(sys.argv) != 4:
            raise SystemExit(
                "--audit-lowering requires lowered MLIR and metadata files"
            )
        ...
        if not re.search(r'bosc_ame\.target\s*=\s*"nr-fpga"', lowered):
            raise SystemExit(
                "NR build rejected: buddy-opt must support and select target=nr-fpga; GEM5 and legacy FPGA artifacts cannot run in this runtime"
            )
        if metadata.get("kind") == "matmul_i8":
            if "bosc_ame.mqma.b.mm" not in lowered or re.search(
                r"\blinalg\.matmul", lowered
            ):
                raise SystemExit(
                    "NR AME build rejected: integer matmul did not lower to bosc_ame.mqma.b.mm (CPU fallback is not an AME validation)"
                )
        return True
    return False
...
def main():
    ...
    p.add_argument(
        "--group",
        choices=["all", "ame", "aux", "rvv", "scalar", "smoke"],
        default="all",
    )
    ...
    for source in sorted(ROOT.glob("*/kernel.mlir")):
        ...
        selected = (
            ...
            or args.group == "ame"
            and kind == "matmul_i8"
            or args.group in ("rvv", "scalar")
            and kind == "matmul_f32"
            or args.group == "aux"
            and kind not in ("matmul_i8", "matmul_f32")
            or args.group == "smoke"
            and name
            in (
                "matmul_3x19x70",
        ...
        if args.cases is not None:
            selected = name in args.cases
        if selected:
            cases.append(name)
    if not cases:
        p.error("no matching operator cases")
    if args.cases and set(args.cases) - set(cases):
        p.error("unknown --case")
    out = (
        ROOT
        / "build"
        / ("suite-" + args.group + ("-host" if args.host else ""))
    )
    ...
    def build(name):
        run(["make", "-C", ROOT / name, target, *makevars])

    with ThreadPoolExecutor(max_workers=args.jobs) as pool:
        list(pool.map(build, cases))
    ...
    for i, name in enumerate(cases):
        ...
        calls.append(
            f'  nr_puts("[{i + 1}/{len(cases)}] {name} BEGIN\\r\\n"); '
            f"failures += {symbol}() != 0;"
        )
        obj = out / (name + ".o")
        run(
        ...
        objects.append(
            ...
            / name
            / build_dir
            / ("kernel.host.ll" if args.host else "kernel.o")
        )
    suite = out / "suite.c"
    suite.write_text(
        '#include "support.h"\n'
...
"""
    ...
    if args.host:
        ...
        obj = out / "suite.o"
        run([*cc, *flags, "-c", suite, "-o", obj], cwd=case_directory)
        objects.append(obj)
        objects += [
            case_directory / path
            for path in shlex.split(variables["RUNTIME_OBJS"])
        ]
        ...
        run(
            [
                *ld,
                "--gc-sections",
                "-T",
                str(Path(variables["NR"]) / "nr.ld"),
                "-Map=" + str(out / "suite.map"),
                "-o",
                elf,
                *objects,
            ],
        ...
        run(
            [
                *shlex.split(variables["PYTHON"]),
                ROOT.parent / "tools/check_nr_elf.py",
                elf,
                "--objdump",
                variables["RISCV_OBJDUMP"],
                "--output",
                out / "elf-audit.json",
            ],
            cwd=case_directory,
        ...
        run(
            [
                *shlex.split(variables["RISCV_OBJCOPY"]),
                "-O",
                "binary",
                elf,
                image,
    ...
    source_files = {
        ...
        ROOT / "tools/host_main.c",
        ROOT / "tools/vectorize_nr.py",
        ROOT.parent / "common/toolchain.mk",
    }
    for name in cases:
        source_files.update(
            ROOT / name / filename
