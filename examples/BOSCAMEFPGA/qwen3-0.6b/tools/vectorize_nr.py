...
def transpose_strided_b(source):
    ...
    def replace(match):
        ...
        count += 1
        transposed = (
            f"memref<{n}x{k}xf32, strided<[{stride}, 1], "
        ...
        return (
            f"{match['indent']}{name} = memref.transpose {match['b']} "
    ...
    if not count or re.search(r"\blinalg\.matmul\s+ins\(", result):
        raise ValueError(
            "expected static FP32 matmul B with strides [1,row_stride>=K]"
        )
    return result
...
def main():
    ...
    if args.metadata is None:
        parser.error(
            "--metadata is required unless --transpose-strided-b is used"
        )
    ...
    if args.prepare_llvm:
        ...
        for line in args.input.read_text().splitlines():
            if (
                ...
                and "@_mlir_ciface_" in line
                and "noimplicitfloat" not in line
            ):
                line = re.sub(
                    r"(\))(?=\s*(?:#[0-9]+\s*)?\{)", r"\1 noimplicitfloat", line
                )
            lines.append(line)
    ...
    option = (
        "FP32_PASS" if metadata["kind"] == "matmul_f32" else "BATCH_FP32_PASS"
    )
    ...
    if "vector.fma" not in result or re.search(
        r"\blinalg\.(?:matmul|batch_matmul)", result
    ):
        args.output.unlink(missing_ok=True)
        raise SystemExit(
            "NR FP32 vectorization did not replace every matrix operation with vector FMA"
        )
