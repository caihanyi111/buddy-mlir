//===- launch.c - Board/host launcher for dequantize_1x151936 -------------===//
//
...
#define M 1
#define N 151936
extern void _mlir_ciface_kernel_dequantize_1x151936(MemRef2 *, MemRef1 *,
                                                    MemRef1 *, MemRef2 *);
int launch(void) {
  ...
  _mlir_ciface_kernel_dequantize_1x151936(&X, &R, &S, &Y);
  unsigned errors = 0;
  float max_error = 0;
  for (int i = 0; i < M; i++)
    for (int j = 0; j < N; j++) {
  ...
  return print_check("dequantize_1x151936", errors, max_error);
}
