//===- launch.c - Board/host launcher for gqa_repeat_8x16x128_to_16x16x128
//---===//
//
...
extern void _mlir_ciface_kernel_gqa_repeat_8x16x128_to_16x16x128(MemRef3 *,
                                                                 MemRef3 *);
int launch(void) {
  int errors = 0;
  float maximum = 0;
  ...
  for (int h = 0; h < 16; ++h)
    for (int t = 0; t < 16; ++t)
      for (int d = 0; d < 128; ++d) {
        compare(out[(h * 16 + t) * 128 + d], x[((h / 2) * 16 + t) * 128 + d], 0,
                0, &errors, &maximum);
      }
