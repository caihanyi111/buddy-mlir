// Dequantize i32 with per-row and per-column scales (dequantize_1x2048, i32 to
...
module {
  func.func @kernel_dequantize_1x2048(
      %x: memref<1x2048xi32>,
      %row: memref<1xf32>,
      %col: memref<2048xf32>,
      %y: memref<1x2048xf32>) attributes {llvm.emit_c_interface} {
    linalg.generic {
        indexing_maps = [#map, #map1, #map2, #map],
        iterator_types = ["parallel", "parallel"]
      }
      ins(%x, %row, %col : memref<1x2048xi32>, memref<1xf32>, memref<2048xf32>)
      outs(%y : memref<1x2048xf32>) {
