//===- launch.c - Board/host launcher for add_16x1024 ---------------------===//
//
...
extern void _mlir_ciface_kernel_add_16x1024(MemRef2 *, MemRef2 *, MemRef2 *);
int launch(void) {
  int errors = 0;
  float maximum = 0;
  float *x = (float *)workspace(0u);
  MemRef2 m_x = make_2(x, 16, 1024);
  float *y = (float *)workspace(65536u);
  MemRef2 m_y = make_2(y, 16, 1024);
  float *out = (float *)workspace(131072u);
  ...
  _mlir_ciface_kernel_add_16x1024(&m_x, &m_y, &m_out);
  for (int i = 0; i < 16384; ++i)
    compare(out[i], x[i] + y[i], 2e-6f, 3e-5f, &errors, &maximum);
  return print_check("add_16x1024", errors, maximum);
}
