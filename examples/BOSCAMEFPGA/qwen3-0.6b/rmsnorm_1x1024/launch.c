//===- launch.c - Board/host launcher for rmsnorm_1x1024 ------------------===//
//
...
extern void _mlir_ciface_kernel_rmsnorm_1x1024(MemRef2 *, MemRef1 *, MemRef1 *,
                                               MemRef2 *);
int launch(void) {
  int errors = 0;
  float maximum = 0;
  float *x = (float *)workspace(0u);
  MemRef2 m_x = make_2(x, 1, 1024);
  float *weight = (float *)workspace(4096u);
  MemRef1 m_weight = make_1(weight, 1024);
  float *sums = (float *)workspace(8192u);
  MemRef1 m_sums = make_1(sums, 1);
  float *out = (float *)workspace(8256u);
  ...
  _mlir_ciface_kernel_rmsnorm_1x1024(&m_x, &m_weight, &m_sums, &m_out);
  for (int r = 0; r < 1; ++r) {
    oracle_float sum = 0;
  ...
  return print_check("rmsnorm_1x1024", errors, maximum);
}
