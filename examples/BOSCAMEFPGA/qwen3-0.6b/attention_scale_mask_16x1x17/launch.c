//===- launch.c - Board/host launcher for attention_scale_mask_16x1x17 ----===//
//
...
extern void _mlir_ciface_kernel_attention_scale_mask_16x1x17(MemRef3 *,
                                                             MemRef3 *);
int launch(void) {
  int errors = 0;
  float maximum = 0;
  ...
  for (int h = 0; h < 16; ++h)
    for (int s = 0; s < 1; ++s)
      for (int t = 0; t < 17; ++t) {
        int i = (h * 1 + s) * 17 + t;
        float expected =
            t <= s + 16 ? x[i] / oracle_sqrt(128.0f) : -__builtin_inff();
