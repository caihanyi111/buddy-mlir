//===- launch.c - Board/host launcher for mul_16x3072 ---------------------===//
//
...
extern void _mlir_ciface_kernel_mul_16x3072(MemRef2 *, MemRef2 *, MemRef2 *);
int launch(void) {
  int errors = 0;
  float maximum = 0;
  float *x = (float *)workspace(0u);
  MemRef2 m_x = make_2(x, 16, 3072);
  float *y = (float *)workspace(196608u);
  MemRef2 m_y = make_2(y, 16, 3072);
  float *out = (float *)workspace(393216u);
  ...
  _mlir_ciface_kernel_mul_16x3072(&m_x, &m_y, &m_out);
  for (int i = 0; i < 49152; ++i)
    compare(out[i], x[i] * y[i], 2e-6f, 3e-5f, &errors, &maximum);
  return print_check("mul_16x3072", errors, maximum);
}
