//===- launch.c - Board/host launcher for kv_cache_update_1x8x128 ---------===//
//
...
extern void _mlir_ciface_kernel_kv_cache_update_1x8x128(MemRef3 *, MemRef3 *);
int launch(void) {
  int errors = 0;
  float maximum = 0;
  float *x = (float *)workspace(0u);
  MemRef3 m_x = make_3(x, 1, 8, 128);
  float *cache = (float *)workspace(4096u);
  MemRef3 m_cache = make_3(cache, 8, 32, 128);
  for (int i = 0; i < 1024; ++i)
    x[i] = (float)((i * 17) % 101 - 50) * 0.0625f;
  for (int i = 0; i < 32768; ++i)
    cache[i] = (float)((i * 3) % 47 - 23) * 0.03125f;
  _mlir_ciface_kernel_kv_cache_update_1x8x128(&m_x, &m_cache);
  for (int h = 0; h < 8; ++h)
    for (int t = 0; t < 32; ++t)
      for (int d = 0; d < 128; ++d) {
        int i = (h * 32 + t) * 128 + d;
        float expected = (float)((i * 3) % 47 - 23) * 0.03125f;
        if (t >= 16 && t < 17)
          expected = x[((t - 16) * 8 + h) * 128 + d];
        compare(cache[i], expected, 0, 0, &errors, &maximum);
      }
  return print_check("kv_cache_update_1x8x128", errors, maximum);
}
