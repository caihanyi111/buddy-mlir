// Per-token quantization from f32 to int8 (quantize_1x2048). Part of the NR
...
module {
  func.func @kernel_quantize_1x2048(
      %x: memref<1x2048xf32>,
      %q: memref<1x2048xi8>,
      %scale: memref<1xf32>) attributes {llvm.emit_c_interface} {
    ...
    linalg.generic {
      ...
      ins(%x : memref<1x2048xf32>)
      outs(%scale : memref<1xf32>) {
    ^bb0(%v: f32, %a: f32):
      %abs = math.absf %v : f32
    ...
    linalg.generic {
      ...
      ins(%x, %scale : memref<1x2048xf32>, memref<1xf32>)
      outs(%q : memref<1x2048xi8>) {
