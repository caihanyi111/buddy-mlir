//===- launch.c - Board/host launcher for rmsnorm_256x128 -----------------===//
//
...
extern void _mlir_ciface_kernel_rmsnorm_256x128(MemRef2 *, MemRef1 *, MemRef1 *,
                                                MemRef2 *);
int launch(void) {
  int errors = 0;
  float maximum = 0;
  float *x = (float *)workspace(0u);
  MemRef2 m_x = make_2(x, 256, 128);
  float *weight = (float *)workspace(131072u);
  MemRef1 m_weight = make_1(weight, 128);
  float *sums = (float *)workspace(131584u);
  MemRef1 m_sums = make_1(sums, 256);
  float *out = (float *)workspace(132608u);
  MemRef2 m_out = make_2(out, 256, 128);
  ...
  _mlir_ciface_kernel_rmsnorm_256x128(&m_x, &m_weight, &m_sums, &m_out);
  for (int r = 0; r < 256; ++r) {
    oracle_float sum = 0;
    for (int k = 0; k < 128; ++k) {
  ...
  return print_check("rmsnorm_256x128", errors, maximum);
}
