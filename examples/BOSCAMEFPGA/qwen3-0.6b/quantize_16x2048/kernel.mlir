// Per-token quantization from f32 to int8 (quantize_16x2048). Part of the NR
...
module {
  func.func @kernel_quantize_16x2048(
      %x: memref<16x2048xf32>,
      %q: memref<16x2048xi8>,
      %scale: memref<16xf32>) attributes {llvm.emit_c_interface} {
    ...
    linalg.fill ins(%z : f32) outs(%scale : memref<16xf32>)
    linalg.generic {
        indexing_maps = [#map, #map1],
        iterator_types = ["parallel", "reduction"]
      }
      ins(%x : memref<16x2048xf32>)
      outs(%scale : memref<16xf32>) {
    ^bb0(%v: f32, %a: f32):
      %abs = math.absf %v : f32
    ...
    linalg.generic {
      ...
      ins(%x, %scale : memref<16x2048xf32>, memref<16xf32>)
      outs(%q : memref<16x2048xi8>) {
