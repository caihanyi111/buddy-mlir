//===- launch.c - Board/host launcher for layout_context_16x1x128 ---------===//
//
...
extern void _mlir_ciface_kernel_layout_context_16x1x128(MemRef3 *, MemRef3 *);
int launch(void) {
  int errors = 0;
  float maximum = 0;
  float *x = (float *)workspace(0u);
  MemRef3 m_x = make_3(x, 16, 1, 128);
  float *out = (float *)workspace(8192u);
  MemRef3 m_out = make_3(out, 1, 16, 128);
  for (int i = 0; i < 2048; ++i) {
    x[i] = (float)((i * 19) % 4093 - 2046) * 0.015625f;
    out[i] = 9999;
  }
  _mlir_ciface_kernel_layout_context_16x1x128(&m_x, &m_out);
  for (int a = 0; a < 16; ++a)
    for (int b = 0; b < 1; ++b)
      for (int d = 0; d < 128; ++d) {
        int index[3] = {a, b, d};
        int input = (a * 1 + b) * 128 + d;
        int output = (index[1] * 16 + index[0]) * 128 + index[2];
        compare(out[output], x[input], 0, 0, &errors, &maximum);
      }
  return print_check("layout_context_16x1x128", errors, maximum);
}
