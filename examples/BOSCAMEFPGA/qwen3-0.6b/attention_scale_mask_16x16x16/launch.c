//===- launch.c - Board/host launcher for attention_scale_mask_16x16x16 ---===//
//
...
extern void _mlir_ciface_kernel_attention_scale_mask_16x16x16(MemRef3 *,
                                                              MemRef3 *);
int launch(void) {
  int errors = 0;
  float maximum = 0;
  ...
  for (int h = 0; h < 16; ++h)
    for (int s = 0; s < 16; ++s)
      for (int t = 0; t < 16; ++t) {
        int i = (h * 16 + s) * 16 + t;
        float expected =
            t <= s + 0 ? x[i] / oracle_sqrt(128.0f) : -__builtin_inff();
