//===- launch.c - Board/host launcher for softmax_16x16x16 ----------------===//
//
...
extern void _mlir_ciface_kernel_softmax_16x16x16(MemRef3 *, MemRef2 *,
                                                 MemRef2 *, MemRef3 *);
int launch(void) {
  int errors = 0;
  float maximum = 0;
  ...
  for (int row = 0; row < 256; ++row) {
    ...
    oracle_float sum = 0;
    for (int k = 0; k < 16; ++k)
      if (x[row * 16 + k] > maximum_input)
        maximum_input = x[row * 16 + k];
    for (int k = 0; k < 16; ++k)
      sum += oracle_exp(x[row * 16 + k] - maximum_input);
